...
import pandas as pd
import ast
from env_with_slot_fixed import RerouteEnv
import os

# 기존 모델 삭제 (선택사항)
if os.path.exists("dqn_model_eta_based_passenger_2.pt"):
    os.remove("dqn_model_eta_based_passenger_2.pt")
...
def train():
    ...
    pd.DataFrame(reward_log).to_csv("train_reward_component_log_passenger.csv", index=False)
    torch.save(model.state_dict(), "dqn_model_eta_based_passenger_2.pt")
